...
public class ConnectionActivity extends Activity {
 private WebView web;private ValueCallback<Uri[]> photos;private Uri cameraPhoto;
 ...
 @Override public boolean onShowFileChooser(WebView v,ValueCallback<Uri[]> cb,FileChooserParams params){if(photos!=null)photos.onReceiveValue(null);photos=cb;try{if(params.isCaptureEnabled()){ContentValues values=new ContentValues();values.put(android.provider.MediaStore.Images.Media.DISPLAY_NAME,"AMC-"+System.currentTimeMillis()+".jpg");values.put(android.provider.MediaStore.Images.Media.MIME_TYPE,"image/jpeg");values.put(android.provider.MediaStore.Images.Media.RELATIVE_PATH,Environment.DIRECTORY_PICTURES+"/AMC");cameraPhoto=getContentResolver().insert(android.provider.MediaStore.Images.Media.EXTERNAL_CONTENT_URI,values);if(cameraPhoto==null)throw new IllegalStateException("No camera destination");Intent capture=new Intent(android.provider.MediaStore.ACTION_IMAGE_CAPTURE);capture.putExtra(android.provider.MediaStore.EXTRA_OUTPUT,cameraPhoto);capture.setClipData(ClipData.newRawUri("AMC",cameraPhoto));capture.addFlags(Intent.FLAG_GRANT_WRITE_URI_PERMISSION|Intent.FLAG_GRANT_READ_URI_PERMISSION);startActivityForResult(capture,11);}else{Intent pick=params.createIntent().putExtra(Intent.EXTRA_ALLOW_MULTIPLE,params.getMode()==FileChooserParams.MODE_OPEN_MULTIPLE);startActivityForResult(Intent.createChooser(pick,"Elegí fotos o un comprobante"),10);}}catch(Exception e){if(cameraPhoto!=null){getContentResolver().delete(cameraPhoto,null,null);cameraPhoto=null;}photos.onReceiveValue(null);photos=null;Toast.makeText(ConnectionActivity.this,"No se pudo abrir la cámara o el selector de fotos.",Toast.LENGTH_LONG).show();}return true;}});
