...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state) {
        ...
        webView.setWebViewClient(new WebViewClient(){@Override public void onPageFinished(WebView view,String url){super.onPageFinished(view,url);sendPushToken(false);}});
    ...
    private void sendPushToken(boolean manual){FirebaseMessaging.getInstance().getToken().addOnSuccessListener(token->{String safe=token.replace("\\","\\\\").replace("\"","\\\"");webView.evaluateJavascript("window.amcNativeToken&&window.amcNativeToken(\""+safe+"\","+manual+")",null);});}
