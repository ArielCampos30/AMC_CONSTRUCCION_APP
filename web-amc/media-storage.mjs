export function mediaStorageFeatures({db,all,put,transaction,objectStore,text,fail,id,now}){
 const remoteDatabase=!!(process.env.NODE_ENV==='test'?process.env.AMC_TEST_DATABASE_URL:process.env.AMC_DATABASE_URL);
 ...
 const writeUploadBundle=({key,user,mime,bytes,miniature,viewer,meta})=>{
  if(remoteDatabase){
   const rows=[
    [key,user.id,mime,bytes],
    ...(miniature?[[key+'-thumb',user.id,'image/jpeg',miniature]]:[]),
    ...(viewer?[[key+'-view',user.id,'image/jpeg',viewer]]:[])
   ],values=rows.map(()=>'(?,?,?,?)').join(','),params=rows.flat();
   db.prepare(`WITH inserted AS (INSERT INTO files(id,owner,mime,body) VALUES ${values} RETURNING id) INSERT INTO docs(id,kind,owner,body) SELECT ?,'fileUpload',?,? WHERE EXISTS (SELECT 1 FROM inserted) ON CONFLICT(id) DO UPDATE SET body=excluded.body,owner=excluded.owner`).run(...params,meta.id,user.id,JSON.stringify(meta));
   return;
  }
  transaction(()=>{
 ...
 const upload=async(user,b)=>{
  ...
  const storedSize=bytes.length+(miniature?.length||0)+(viewer?.length||0),quotaStarted=Date.now(),quota=db.prepare('SELECT coalesce(sum(length(body)),0) AS total, coalesce(sum(CASE WHEN owner=? THEN length(body) ELSE 0 END),0) AS used FROM files').get(user.id),quotaMs=Date.now()-quotaStarted;
  if(Number(quota.total||0)+storedSize>150*1024*1024)fail(413,'El almacenamiento de archivos de la prueba está completo. AMC debe ampliar o revisar el espacio.');
  if(Number(quota.used||0)+storedSize>100*1024*1024)fail(413,'Alcanzaste el límite de archivos de esta versión.');
  ...
  try{
   ...
   writeUploadBundle({key,user,mime,bytes,miniature,viewer,meta});
