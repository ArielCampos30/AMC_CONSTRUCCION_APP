...
const signatureValid=(raw,signature,secret)=>{
...
export function whatsappCloudFeatures({db,all,put,transaction,requireAdmin,readRaw,send,fail,now,sha,config={}}){
 ...
 const saveMessage=(entry,value,message)=>{
  const metaId=String(message?.id||'');if(!metaId)return;
  const docId=messageId(metaId),existing=db.prepare("SELECT body FROM docs WHERE kind='whatsappMessage' AND id=?").get(docId);
  if(existing)return;
  const contact=contactFor(value,message?.from),remoteId=String(message?.from||contact?.wa_id||contact?.id||'');
  const phoneCandidate=String(contact?.wa_id||message?.from||'');
  put('whatsappMessage','',{id:docId,metaMessageId:metaId,direction:'inbound',remoteId,phone:phoneCandidate,name:String(contact?.profile?.name||'').slice(0,160),type:String(message?.type||'unknown').slice(0,80),text:messagePreview(message),messageAt:asIso(message?.timestamp)||now(),receivedAt:now(),phoneNumberId:String(value?.metadata?.phone_number_id||''),businessAccountId:String(entry?.id||''),status:'received',source:'meta-webhook'});
 };
 const saveStatus=status=>{
  const metaId=String(status?.id||'');if(!metaId)return;
  const statusName=String(status?.status||'unknown').slice(0,80),timestamp=String(status?.timestamp||'');
  const docId=statusId(metaId,statusName,timestamp);if(db.prepare("SELECT id FROM docs WHERE kind='whatsappStatus' AND id=?").get(docId))return;
  put('whatsappStatus','',{id:docId,metaMessageId:metaId,status:statusName,statusAt:asIso(timestamp)||now(),recipientId:String(status?.recipient_id||''),conversationId:String(status?.conversation?.id||''),pricingCategory:String(status?.pricing?.category||''),receivedAt:now()});
  const messageRow=db.prepare("SELECT body FROM docs WHERE kind='whatsappMessage' AND id=?").get(messageId(metaId));
  if(messageRow){const message=JSON.parse(messageRow.body);put('whatsappMessage','',{...message,status:statusName,statusAt:asIso(timestamp)||now()});}
 };
 const ingest=payload=>transaction(()=>{
  ...
  for(const entry of Array.isArray(payload.entry)?payload.entry:[]){
   ...
   for(const change of Array.isArray(entry?.changes)?entry.changes:[]){
    ...
    for(const message of Array.isArray(value.messages)?value.messages:[]){saveMessage(entry,value,message);messages++;}
    for(const status of Array.isArray(value.statuses)?value.statuses:[]){saveStatus(status);statuses++;}
   }
  }
 ...
 const publicRoute=async({req,res,p,method,url})=>{
  ...
  if(!signatureValid(raw,signature,appSecret))fail(403,'Firma de webhook inválida.');
 ...
 return {publicRoute,adminRoute,configured,signatureValid};
}
