...
export class PostgresDatabase{
 constructor(url,{schema='amc_data',test=false,caFile}={}){
  ...
  this.workerData={url,schema,test,caFile,control:this.control,bytes:this.bytes};this.closed=false;this.failed=false;
  this.startWorker();
 }
 startWorker(){
 ...
 query(sql,params=[]){if(this.closed)throw Object.assign(Error('Base de datos cerrada.'),{status:503});if(this.failed)this.recoverWorker();Atomics.store(this.state,0,0);try{this.worker.postMessage({sql,params});}catch{this.failed=true;throw Object.assign(Error('Base de datos sin conexión.'),{status:503});}return this.receive();}
 ...
 close(){if(this.closed)return;if(!this.failed){try{this.query('__close');}catch{}}this.closed=true;try{this.worker?.terminate();}catch{}}
}
