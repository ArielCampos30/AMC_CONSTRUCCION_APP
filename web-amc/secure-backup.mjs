...
const emptyStats=()=>({users:0,docs:0,files:0,config:0,storageBytes:0});
...
function addStats(stats,table,row,encodedFile=false){
 stats[table]++;
 if(table==='files')stats.storageBytes+=encodedFile?Buffer.from(row.body,'base64').length:Buffer.from(row.body).length;
}
...
export function exportBackup(db,file,password){
 const header=Buffer.concat([MAGIC,randomBytes(16)]),key=keyFor(password,header.subarray(8)),stats=emptyStats();
 ...
 try{writeAll(fd,header);db.exec(typeof db.query==='function'?'BEGIN ISOLATION LEVEL REPEATABLE READ READ ONLY':'BEGIN IMMEDIATE');begun=true;
  for(const [table,columns] of Object.entries(tables)){
   ...
   if(table==='files'){for(const {id} of db.prepare('SELECT id FROM files').all()){const row=db.prepare('SELECT id,owner,mime,body FROM files WHERE id=?').get(id);addStats(stats,table,row);frame({table,row:{...row,body:Buffer.from(row.body).toString('base64')}});}}
   else for(const row of db.prepare('SELECT '+columns.join(',')+' FROM '+table+(table==='docs'?' ORDER BY rowid ASC':'')).all()){addStats(stats,table,row);frame({table,row});}
  }
  db.exec('COMMIT');begun=false;frame({end:true,count});done=true;return {records:count-1,stats};
 }finally{if(begun)db.exec('ROLLBACK');closeSync(fd);key.fill(0);if(!done)unlinkSync(file);}
}

function scan(fd,password,consume=()=>{}){
 const header=exact(fd,24);if(!header.subarray(0,8).equals(MAGIC))throw Error('Formato de respaldo desconocido.');const key=keyFor(password,header.subarray(8)),stats=emptyStats();let count=0;
 try{while(true){const size=exact(fd,4).readUInt32BE();if(size<1||size>10*1024*1024)throw Error('Tamaño de respaldo inválido.');const iv=exact(fd,12),tag=exact(fd,16),encrypted=exact(fd,size),decipher=createDecipheriv('aes-256-gcm',key,iv);decipher.setAAD(aad(header,count));decipher.setAuthTag(tag);const record=JSON.parse(Buffer.concat([decipher.update(encrypted),decipher.final()]).toString('utf8'));
   if(record.end===true){if(record.count!==count||readSync(fd,Buffer.alloc(1),0,1)!==0)throw Error('Respaldo incompleto o alterado.');return {records:count,stats};}
   validate(record);addStats(stats,record.table,record.row,true);consume(record);count++;
