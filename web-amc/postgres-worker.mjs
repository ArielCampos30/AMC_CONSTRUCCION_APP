...
let client,pg,clientConfig,failed=true,closing=false;
...
parentPort.on('message',async({sql,params=[]})=>{
 if(sql==='__close'){closing=true;try{await client?.end();}catch{}failed=true;reply({ok:true});parentPort.close();return;}
