...
test('client, admin and employee notices become read when their related content is seen',async()=>{
 ...
 try{
  ...
  clientState=await client.call('/api/state');assert.equal(clientState.notices.some(n=>n.url==='/#presupuesto/'+quote.id&&!n.read),false);
  const quotePdf=await admin.call('/api/upload',{mime:'application/pdf',base64:Buffer.from('%PDF-1.4 notice').toString('base64')},201);
