...
test('9.4 restaura una copia cifrada en una base aislada y compara datos y archivos',()=>{
 ...
 try{
  ...
  assert.equal(exported.stats.users,1);
  assert.equal(exported.stats.docs,1);
  assert.equal(exported.stats.files,1);
  assert.equal(exported.stats.storageBytes,8);
  ...
  const recovery=verifyRestoredCopy({file:backup,password,target});

  assert.equal(recovery.records,exported.records);
  assert.deepEqual({users:recovery.users,docs:recovery.docs,files:recovery.files,config:recovery.config,storageBytes:recovery.storageBytes},exported.stats);
  assert.equal(process.env.AMC_DATABASE_URL,'postgres://must-not-be-used');
  assert.equal(process.env.AMC_TEST_DATABASE_URL,'postgres://must-not-be-used');
  assert.equal(process.env.AMC_ADMIN_EMAIL,'must-not-bootstrap@test.test');
  assert.equal(process.env.AMC_ADMIN_PASSWORD,'Must-Not-Bootstrap-2026!');
 }finally{
