...
test('runtime comercial guarda, registra contacto y abre WhatsApp sin afirmar entrega',async()=>{
 const calls=[],opened=[],runtime=createCommercialFollowupRuntime({api:async(path,body)=>{calls.push({path,body});return {ok:true};},openExternal:url=>opened.push(url)}),form={classList:{contains:value=>value==='commercial-followup-form'},dataset:{id:'request 1'}};
 ...
 assert.equal(await runtime.submit(form,{nextAction:'Esperar respuesta',nextActionDay:'2026-09-20',note:'Seguimiento por WhatsApp.'},{value:'whatsapp',dataset:{whatsappUrl:'https://wa.me/5493548000011?text=Hola%20Cliente'}}),true);
 assert.deepEqual(opened,['https://wa.me/5493548000011?text=Hola%20Cliente']);
 assert.deepEqual(calls,[
  ...
  {path:'/api/admin/commercial-followups/request%201',body:{nextAction:'Esperar respuesta',nextActionDay:'2026-09-20',note:'Seguimiento por WhatsApp.',action:'whatsapp_opened'}}
 ]);
 await assert.rejects(()=>runtime.submit(form,{},{value:'whatsapp',dataset:{whatsappUrl:'javascript:alert(1)'}}),/WhatsApp válido/);
});

test('Admin guarda un único seguimiento por solicitud y registra WhatsApp iniciado sin exponerlo al cliente',async()=>{
 ...
 try{
  ...
  const third=await admin.call('/api/admin/commercial-followups/'+request.id,{nextAction:'Esperar respuesta',nextActionDay:'2026-09-20',note:'Se abrió la conversación comercial.',action:'whatsapp_opened'});
  assert.ok(third.lastContactAt);assert.equal(third.history.length,3);assert.equal(third.history[2].type,'whatsapp_opened');assert.equal(third.history[2].channel,'WhatsApp');
  const adminState=await admin.call('/api/state');assert.equal(adminState.commercialFollowups.length,1);assert.equal(adminState.commercialFollowups[0].nextAction,'Esperar respuesta');assert.equal(adminState.commercialFollowups[0].history.length,3);
