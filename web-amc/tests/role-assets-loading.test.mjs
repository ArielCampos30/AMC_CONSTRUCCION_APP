...
test('mapa de assets mantiene separación estricta entre Admin, Empleado, Cliente y público',()=>{
 assert.deepEqual(ROLE_ASSETS.admin.styles,['/admin-v3.css','/admin-appearance.css','/admin-dashboard.css','/admin-commercial.css']);
...
test('carga sólo el rol activo, limpia al salir y puede reinsertar CSS sin reimportar módulos',async()=>{
 ...
 assert.deepEqual(links.map(link=>link.href).sort(),['/employee-staff-chat.css','/employee-v4.css','/operational-ux.css']);
 ...
 assert.deepEqual(links.map(link=>link.href).sort(),['/admin-appearance.css','/admin-commercial.css','/admin-dashboard.css','/admin-v3.css']);
...
test('HTML y runtime inicial ya no descargan los assets de todos los roles',async()=>{
 ...
 assert.doesNotMatch(html,/href="\/(?:admin-v3|admin-appearance|admin-dashboard|admin-commercial|employee-v4|employee-staff-chat|client-v5|operational-ux)\.css"/);
 ...
 assert.doesNotMatch(app,/adminStyle|href='\/admin-v3\.css'|href="\/admin-v3\.css"/);
 ...
 assert.doesNotMatch(sw,/admin-v3\.css|admin-appearance\.css|admin-dashboard\.css|admin-commercial\.css|employee-v4\.css|employee-staff-chat\.css|client-v5\.css|operational-ux\.css/);
});
