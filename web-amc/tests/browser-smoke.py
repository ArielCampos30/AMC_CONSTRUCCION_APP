...
def quote_shell_audit():
    return js("""const host=document.querySelector('.quote-wizard-host'),header=host?.querySelector('.quote-wizard-header'),steps=host?.querySelector('.quote-wizard-steps'),title=host?.querySelector('#quote-wizard-title'),close=host?.querySelector('.quote-wizard-close'),icon=close?.querySelector('svg'),controls=host?.querySelector('.quote-wizard-controls'),nav=document.querySelector('.bottom-nav'),bubble=document.querySelector('.floating-chat-button'),pricing=[...host?.querySelectorAll('.quote-pricing-tabs button')||[]],client=host?.querySelector('[data-qw-client]'),addClient=host?.querySelector('[data-qw-new-client]');const rect=n=>n?.getBoundingClientRect(),intersects=(a,b,t=1)=>a&&b&&a.left<b.right-t&&a.right>b.left+t&&a.top<b.bottom-t&&a.bottom>b.top+t,hr=rect(header),sr=rect(steps),tr=rect(title),cr=rect(close),ir=rect(icon),nr=rect(nav),br=rect(controls),pairOverlap=pricing.some((button,index)=>pricing.slice(index+1).some(other=>intersects(rect(button),rect(other)))),relevamiento=pricing.find(button=>button.textContent.trim()==='Relevamiento'),keyNodes=[header,steps,controls,client,addClient,...pricing].filter(Boolean),outside=keyNodes.filter(node=>{const r=rect(node);return r.left<-2||r.right>innerWidth+2}).map(node=>node.className||node.tagName);return {overflow:document.documentElement.scrollWidth>innerWidth+2,headerFlow:!!hr&&!!sr&&hr.bottom<=sr.top+1,titleCloseOverlap:intersects(tr,cr),closeRound:!!cr&&Math.abs(cr.width-cr.height)<=1,iconCentered:!!cr&&!!ir&&Math.abs((cr.left+cr.right)/2-(ir.left+ir.right)/2)<=1&&Math.abs((cr.top+cr.bottom)/2-(ir.top+ir.bottom)/2)<=1,pricingOverlap:pairOverlap,relevamientoFits:!relevamiento||(relevamiento.scrollWidth<=relevamiento.clientWidth+2&&relevamiento.scrollHeight<=relevamiento.clientHeight+2),controlsClearNav:!nr||getComputedStyle(nav).display==='none'||!br||br.bottom<=nr.top+1,chatVisible:!!bubble&&bubble.offsetParent!==null,outside,clientOverlap:intersects(rect(client),rect(addClient))};""")
