...
test('PWA release metadata, safe cache and iPhone install help are present',()=>{
 ...
 assert.match(ios,/Agregar a pantalla de inicio/);assert.match(ios,/amc-ios-install-dismissed/);assert.doesNotMatch(sw,/api\//);assert.match(sw,/AMC-offline-shell-v8/);assert.match(sw,/amc-theme\.css/);
});
