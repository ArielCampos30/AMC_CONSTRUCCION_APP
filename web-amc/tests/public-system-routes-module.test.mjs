...
test('salud y configuración pública quedan fuera del router principal',async()=>{
 ...
 assert.match(composition,/performanceHealth:performance\.health/);
 ...
 assert.match(routes,/performanceHealth\(\)/);
 assert.match(routes,/webPushKey:keys\.publicKey/);
});

test('rutas públicas conservan respuesta de salud, rendimiento y configuración',()=>{
 const sent=[];
 const route=publicSystemRoutes({
  ...
  performanceHealth:()=>({stateRequests15m:4,stateP95Ms:720,stateMaxMs:900,mediaRequests15m:2,mediaP95Ms:1800,mediaMaxMs:2200}),
 ...
 assert.equal(sent[0].status,200);assert.equal(sent[0].data.database,'available');assert.equal(sent[0].data.driver,'postgresql');assert.equal(sent[0].data.version,'abc1234');assert.equal(sent[0].data.errors5xx15m,2);assert.equal(sent[0].data.backup,'ok');assert.equal(sent[0].data.stateP95Ms,720);assert.equal(sent[0].data.mediaMaxMs,2200);
