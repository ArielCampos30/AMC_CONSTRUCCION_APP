...
test('chat móvil conserva el compositor, sigue al teclado y evita avisos del hilo visible',async()=>{
 const [runtime,viewportRuntime,routeRuntime,mobileStyles,confirm,index,manifest,activity,push,notices]=await Promise.all([
 ...
 assert.doesNotMatch(runtime,/visualViewport|amc-keyboard-open|dismissComposerAfterSend/);
 assert.match(viewportRuntime,/window\.visualViewport/);
 assert.match(viewportRuntime,/amc-keyboard-open/);
 assert.match(viewportRuntime,/dismissComposerAfterSend/);
...
test('presupuesto avisa sólo al cliente cuando el PDF ya está disponible y el mismo token Android pertenece a la última cuenta que lo registra',async()=>{
 ...
 try{
  ...
  assert.equal((await client.call('/api/state')).notices.some(n=>n.url==='/#presupuesto/'+quote.id&&n.title==='Tu presupuesto está listo'),false);
