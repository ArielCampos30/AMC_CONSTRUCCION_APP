...
test('chat móvil conserva compositor, teclado, push nativo y supresión del hilo visible con ownership separado',async()=>{
 ...
 assert.match(pushRuntime,/hashchange/);
 assert.match(pushRuntime,/pageshow/);
 assert.match(pushRuntime,/visibilitychange/);
 ...
 assert.match(activity,/sendPushToken\(false\)/);
