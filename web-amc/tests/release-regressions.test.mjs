...
test('editar presupuesto restaura el modelo persistido y el precio por quoteId',()=>{
  const features=read('../public/features-ui.js'),runtime=read('../public/quote-tools-runtime.js'),wizard=read('../public/quote-wizard.js');
  assert.match(features,/ensureQuoteTools\('cotizador'\)\.then\(runtime=>runtime\.openEditor\(requestId,quoteId,mode\)\)/);assert.match(runtime,/openEditor\(requestId='',quoteId='',mode=''\)\{wizard\.open\(requestId,quoteId,mode\);deps\.navigate\('cotizador'\);\}/);assert.match(wizard,/const quote=quoteId\?quotes\(\)\.find\(item=>item\.id===quoteId\):null,editable=quote\?\.adminModel\?\.schemaVersion===1\?quote\.adminModel:null/);assert.match(wizard,/const quoteItems=editable\?\.works\|\|quote\?\.items\|\|\[\]/);assert.match(wizard,/works=quoteItems\.map\(item=>createWork\(item\)\)/);assert.match(wizard,/const stored=amount\(editable\?\.finalPrice\?\?quote\.amcClientPrice\?\?quote\.total,0\)/);assert.match(wizard,/if\(stored>0\)\{finalPrice=stored;finalPriceManual=/);
});
