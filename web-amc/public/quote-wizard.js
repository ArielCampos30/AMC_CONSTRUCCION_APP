...
export function createQuoteWizard({getState,isAdmin,esc,navigate,toast,api,refresh}){
 ...
 function referenceResolved(work){
  ...
  if(work.tariffKind==='visit-pending')return false;
  if(work.tariffKind==='jornal')return true;
 ...
 function tariffSuggestionMarkup(work){
  ...
  if(result.status==='visit')return `<div class="quote-price-state warning"><strong>Este trabajo necesita más información.</strong><p>Completá el relevamiento fuera del presupuesto y, cuando tengas los datos, elegí Tarifario, Manual o Jornal para asignarle un precio real.</p></div>`;
  ...
  return `<div class="quote-price-state"><p>No encontré coincidencias todavía. Seguí escribiendo o elegí Manual o Jornal.</p></div>`;
 }
 ...
 function legacyVisitPricing(){return `<div class="quote-price-state warning"><strong>Este trabajo quedó pendiente de un relevamiento anterior.</strong><p>Para continuar, elegí Tarifario, Manual o Jornal y cargá un precio real. El trabajo y su descripción no se pierden.</p></div>`;}
 function pricingPanel(work){
  const legacyVisit=work.tariffKind==='visit-pending',tariffActive=!['manual-reference','jornal','visit-pending'].includes(work.tariffKind),manual=work.tariffKind==='manual-reference',jornal=work.tariffKind==='jornal';
  const tabs=`<div class="quote-pricing-tabs" role="group" aria-label="Forma de calcular el trabajo"><button type="button" class="${tariffActive?'active':''}" data-qw-pricing-mode="tariff" data-qw-work-id="${esc(work.id)}">Tarifario</button><button type="button" class="${manual?'active':''}" data-qw-pricing-mode="manual" data-qw-work-id="${esc(work.id)}">Manual</button><button type="button" class="${jornal?'active':''}" data-qw-pricing-mode="jornal" data-qw-work-id="${esc(work.id)}">Jornal</button></div>`;
  const content=manual?manualPricing(work):jornal?jornalPricing(work):legacyVisit?legacyVisitPricing():tariffPricing(work);
  return `<section class="quote-pricing">${tariffActive?`${content}${tabs}`:`${tabs}${content}`}</section>`;
 }
 ...
 function markup(){const titles=['','Nuevo presupuesto','Trabajos y precios','Costos y rentabilidad','Revisar presupuesto'];return `<section class="quote-wizard-page quote-wizard-stage-${stage}" aria-labelledby="quote-wizard-title" tabindex="-1"><header class="quote-wizard-header"><div><span class="eyebrow">COTIZADOR AMC</span><h1 id="quote-wizard-title">${titles[stage]}</h1></div><button type="button" class="quote-wizard-close" data-qw-close aria-label="Salir del cotizador"><svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M6 6l12 12M18 6L6 18"/></svg></button></header>${phaseNav()}<div class="quote-wizard-content">${body()}</div>${controls()}</section>`;}
 ...
 function updateEconomicPreview(){
  ...
  const reset=host.querySelector('[data-qw-reset-final-price]');if(reset){const usingReference=!finalPriceManual;reset.disabled=usingReference||automaticCommercial(rows)<=0;reset.textContent=usingReference?'Referencia aplicada':'Usar referencia';}
