...
export function createQuoteWizard({getState,isAdmin,esc,navigate,toast}){
 ...
 function open(nextRequest='',nextQuote='',nextMode=''){
  rememberOrigin();requestId=nextRequest||'';quoteId=nextQuote||'';mode=nextMode||'';step=1;resetWorks();ensureTariffs();
 }
 ...
 function worksStep(){const r=selectedRequest();return `<div class="quote-wizard-step"><div class="quote-wizard-step-copy"><span class="eyebrow">PASO 2 DE 7</span><h2>Trabajos a presupuestar</h2><p>Armá la lista de trabajos que realmente forman parte de esta obra. AMC va a buscar únicamente las referencias relacionadas en tu Tarifario, sin mostrarte el catálogo completo dentro del asistente.</p></div>${requestCard(r)}${r?.description?`<div class="quote-wizard-request-note"><span>Detalle pedido por el cliente</span><p>${esc(r.description)}</p></div>`:''}<div class="quote-wizard-suggestions">${suggestions(r).map(value=>`<button type="button" data-qw-suggestion="${esc(value)}">+ ${esc(value)}</button>`).join('')}</div><div class="quote-wizard-work-list">${workRows()}</div><button type="button" class="quote-wizard-add" data-qw-add-work>＋ Agregar otro trabajo</button></div>`;}
 ...
 function render(){if(!isAdmin())return '<section class="panel"><h2>Acceso exclusivo de AMC</h2><p>Esta herramienta está disponible sólo para Administración.</p></section>';choosePrefilledRequest();ensureTariffs();return `<div class="quote-wizard-host">${markup()}</div>`;}
 ...
 async function ensureTariffs(){
  ...
  tariffState='loading';tariffError='';
  try{
   const response=await fetch('/api/estimator-tariffs',{method:'GET',credentials:'same-origin',headers:{Accept:'application/json'}});
   const payload=await response.json().catch(()=>({}));
   if(!response.ok)throw new Error(payload.error||'No se pudo consultar el Tarifario.');
   tariffs=Array.isArray(payload.items)?payload.items:[];tariffState='ready';
  }catch(error){tariffs=[];tariffState='error';tariffError=String(error?.message||'No se pudo consultar el Tarifario.');}
  if(step===3&&document.querySelector('.quote-wizard-host'))paint();
 }
 ...
 document.addEventListener('click',event=>{
  ...
  const next=event.target.closest('[data-qw-next]');if(next&&!next.disabled&&canAdvance()){if(step<4){step++;if(step>=2)initialiseWorks();if(step===3)ensureTariffs();paint();}return;}
 ...
 function handleWorkInput(target){
  ...
  updateAdvanceButton();updateCostPreview();updateLaborPreview();
 }
 ...
 function afterRender(page){if(page!=='cotizador')return;choosePrefilledRequest();ensureTariffs();requestAnimationFrame(()=>document.querySelector('.quote-wizard-dialog')?.focus?.({preventScroll:true}));}
