import {DEFAULT_QUOTE_SETTINGS,amount,normaliseWork,applyTariffSelection,workDirectCost,directCostTotal,workEffectiveLaborCost,workLaborCost,laborCostTotal,internalCostTotal,jornalReference,commercialReferenceTotal,economicSnapshot,findTariffMatches} from './quote-wizard-model.js';
...
export function createQuoteWizard({getState,isAdmin,esc,navigate,toast}){
 ...
 function costWorkCard(work,index){
  ...
  return `<article class="quote-cost-work-card"><header><div><span class="eyebrow">TRABAJO ${index+1}</span><h3>${esc(work.description||'Trabajo sin nombre')}</h3></div><strong data-qw-work-internal="${esc(work.id)}">${money(workDirectCost(work)+effective)}</strong></header><div class="quote-wizard-fields quote-wizard-fields-3"><label>Materiales<input type="number" min="0" step="100" value="${work.materials}" data-qw-work-input data-qw-key="materials" data-qw-work-id="${esc(work.id)}"></label><label>Herramientas / consumibles<input type="number" min="0" step="100" value="${work.tools}" data-qw-work-input data-qw-key="tools" data-qw-work-id="${esc(work.id)}"></label><label>Otros / contingencia<input type="number" min="0" step="100" value="${work.other}" data-qw-work-input data-qw-key="other" data-qw-work-id="${esc(work.id)}"></label><label>Operarios<input type="number" min="1" step="1" value="${work.workers}" data-qw-work-input data-qw-key="workers" data-qw-work-id="${esc(work.id)}"></label><label>Días<input type="number" min="1" step="1" value="${work.days}" data-qw-work-input data-qw-key="days" data-qw-work-id="${esc(work.id)}"></label><label>Mano de obra explícita <small>Opcional: si es mayor a $ 0 reemplaza la estimada.</small><input type="number" min="0" step="100" value="${work.labor}" data-qw-work-input data-qw-key="labor" data-qw-work-id="${esc(work.id)}"></label></div><div class="quote-labor-breakdown"><span>Estimada: <strong>${money(estimated)}</strong></span><span>Explícita: <strong>${explicit>0?money(explicit):'No cargada'}</strong></span><span>Efectiva: <strong data-qw-effective-labor="${esc(work.id)}">${money(effective)}</strong></span></div><label class="quote-cost-confirm"><input type="checkbox" data-qw-cost-confirm="${esc(work.id)}" ${work.costsConfirmed?'checked':''}> Confirmé los costos de este trabajo, incluso si algún valor es $ 0.</label></article>`;
 }
