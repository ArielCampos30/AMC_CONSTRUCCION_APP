...
if(embedded){banner.style.margin='0 12px 16px';banner.querySelector('a').onclick=e=>{e.preventDefault();window.parent.postMessage({type:'amc:navigate'},location.origin);};}if(window.AMCOnline){banner.querySelector('a').onclick=e=>{e.preventDefault();window.AMCOnline.open();};}
 ...
 document.getElementById('share-whatsapp').onclick=()=>{const r=selectedRequest();if(!r?.phone)throw Error('Este cliente no tiene teléfono para compartir por WhatsApp.');const raw=String(r.phone).replace(/\D/g,'').replace(/^0+/,'');const phone=raw.startsWith('549')?raw:raw.startsWith('54')?'549'+raw.slice(2):'549'+raw;const message='Hola '+r.name+', te compartimos el presupuesto '+draft.number+' de AMC Construcciones y Arreglos. Total: '+money(clientTotal(draft))+'. Quedamos atentos a cualquier consulta.';window.open('https://wa.me/'+phone+'?text='+encodeURIComponent(message),'_blank','noopener');};
