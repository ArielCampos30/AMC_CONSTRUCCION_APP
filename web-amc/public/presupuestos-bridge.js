...
if(embedded){banner.style.margin='0 12px 16px';banner.querySelector('a').onclick=e=>{e.preventDefault();window.parent.postMessage({type:'amc:navigate'},location.origin);};}if(window.AMCOnline){banner.querySelector('a').onclick=e=>{e.preventDefault();window.AMCOnline.open();};}
 ...
 async function generatePendingPdf(quoteId){
   ...
   try{
     ...
     const attached=await createAndAttach({quoteId,document:quoteForDocument(draft),makePdf,blobToBase64,request});
 ...
 publicQuote.version=await contentVersion(publicQuote);const sent=await request('/api/quotes',publicQuote);if(!['Enviado','Guardado'].includes(sent.status))throw Error('No se pudo '+action+' este presupuesto.');draft.amcQuoteId=sent.id;persistCurrentSilently();let pdfPending=true;try{await createAndAttach({quoteId:sent.id,document:quoteForDocument(draft),makePdf,blobToBase64,request});pdfPending=false;}catch(pdfError){console.error('El presupuesto quedó guardado, pero el PDF quedó pendiente.',pdfError);}syncDeliveryUi();status().textContent=registered?(pdfPending?'Presupuesto enviado. PDF pendiente de generar.':'Presupuesto enviado. El cliente ya puede verlo y responder desde su cuenta.'):(pdfPending?'Presupuesto guardado. PDF pendiente de generar.':'Presupuesto guardado. Compartilo o registrá su entrega.');if(embedded)window.parent.postMessage({type:registered?'amc:sent':'amc:saved',quoteId:sent.id,clientName:r.name,pdfPending},location.origin);
