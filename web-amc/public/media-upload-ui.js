export function createMediaUploadUI({getCsrf}){
 ...
 async function decodeImageSource(file){
  try{
   ...
   try{
    await new Promise((resolve,reject)=>{image.onload=resolve;image.onerror=()=>reject(Error('decode'));image.src=url;});
 ...
 async function renderJpegVariant(source,width,height,long,maxLong,quality){
  const scale=Math.min(1,maxLong/long),canvas=document.createElement('canvas');
 ...
 function uploadWithProgress(file,thumbnail,onProgress){
  return new Promise((resolve,reject)=>{
   ...
   xhr.onload=()=>{
    let result;
    try{result=JSON.parse(xhr.responseText);}catch{return reject(Error('No pudimos confirmar la subida. Volvé a intentar.'));}
    if(xhr.status>=200&&xhr.status<300)resolve({...result,uploadMs:Math.round(performance.now()-started)});
    else reject(Error(result.error||'No se pudo subir la foto.'));
   };
   xhr.onerror=()=>reject(Error('Se cortó la conexión. La foto sigue adjunta; volvé a intentar.'));
   xhr.ontimeout=()=>reject(Error('La subida tardó demasiado. Revisá la conexión y volvé a intentar.'));
   xhr.send(data);
  });
 }
 async function upload(file,onProgress,showBusy=true){
  if(!file||!file.size)throw Error('Elegí una foto.');
  ...
  try{
   ...
   if(showBusy)window.AMCBusy?.stop();
  }
 }
