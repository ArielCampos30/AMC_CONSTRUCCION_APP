export function createAdminDashboardUI({getState,heading,closureNeedsAction,esc=String,date=value=>value||''}){
 ...
 function searchRows(state,query){
  const term=normalize(query);if(term.length<2)return [];
  const rows=[];
  const add=(kind,title,meta,href,search)=>{if(normalize(search).includes(term))rows.push({kind,title,meta,href});};
 ...
 function searchResultsMarkup(query){
  ...
  if(term.length<2)return '<p class="muted admin-global-search-hint">Escribí al menos 2 caracteres para buscar.</p>';
  const rows=searchRows(getState(),term);
  if(!rows.length)return '<p class="muted admin-global-search-hint">No encontramos clientes, solicitudes, presupuestos ni obras con esa búsqueda.</p>';
  return rows.map(row=>`<a class="admin-global-search-row" href="${row.href}"><span class="admin-search-type">${esc(row.kind)}</span><strong>${esc(row.title)}</strong><small>${esc(row.meta||'')}</small><b>→</b></a>`).join('');
 }
 ...
 dashboard.search=query=>{const target=globalThis.document?.getElementById('admin-global-search-results');if(!target)return false;target.innerHTML=searchResultsMarkup(query);return true;};
