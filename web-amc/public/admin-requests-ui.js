export function createAdminRequestsUI({getState,getFilter,adminChip,adminBadge,quoteCanEdit,quoteCanRevise,heading,esc,date,empty}){
 const render=()=>{
  ...
  return heading('ENTRADAS','Solicitudes','Las que necesitan una decisión aparecen primero.')+`<nav class="admin-v3-chips">${['Nuevas','Revisando','Visita pendiente','Presupuestadas','No tomadas','Todas'].map(value=>adminChip(value,'request',filter)).join('')}</nav><section class="admin-v3-list">${rows.map(request=>{
   const quotes=(state.quotes||[]).filter(quote=>quote.requestId===request.id),latest=quotes.at(-1),work=(state.works||[]).find(item=>item.requestId===request.id),sourceParts=request.source==='landing'?['Landing',request.utmSource,request.utmCampaign].filter(Boolean):[],sourceMeta=sourceParts.length?`<span>Origen: ${esc(sourceParts.join(' · '))}</span>`:'',
    operationalActions=work?`<a href="#obra-admin/${encodeURIComponent(work.id)}">Ver obra</a>`:request.status==='No tomada'?'':[
    ...
    ].filter(Boolean).join(''),trashAction=!work?`<button class="danger" data-maintenance-action="trash-request" data-id="${request.id}">Mover a Papelera</button>`:'',menuActions=operationalActions+trashAction;
   return `<article class="admin-v3-card"><div class="title-row"><div><h2>${esc(request.name)}</h2><p>${esc(request.description.slice(0,130))}</p></div>${adminBadge(request.status)}</div><div class="admin-v3-meta"><span>${esc((request.services||[request.service]).filter(Boolean).join(' · '))}</span><span>${esc(request.town)}</span><span>${date(request.date)}</span><span>${(request.photos||[]).length} fotos</span>${sourceMeta}</div>${request.statusReason?`<p class="admin-v3-reason"><strong>${esc(request.statusReason)}</strong>${request.statusComment?' · '+esc(request.statusComment):''}</p>`:''}<div class="admin-v3-actions"><a class="primary" href="#solicitud/${encodeURIComponent(request.id)}">Ver solicitud</a>${menuActions?`<details><summary>⋮</summary><div>${menuActions}</div></details>`:''}</div></article>`;
